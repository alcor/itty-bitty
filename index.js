...
window.onhashchange = window.onload = function() {
  ...
  if (hash.length < 3) {
    ...
    document.title = title.length ? decodeURIComponent(title.replace(/_/g, " ")) : location.hostname;
    ...
    if ((isEdge || isIE) && location.href.length == 2083) {
      document.getElementById('warning').innerHTML = 'Edge only supports shorter URLs (maximum 2083 bytes).<br>Larger sites may require a different browser.<br><a href="http://reference.bitty.site">Learn more</a>';
    };
